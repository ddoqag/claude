...
async function listRepoFiles(args) {
...
async function fileMetrics(args) {
    const { filePath } = args;
    try {
...
function calculateHealthScore(complexity, functions, commentRatio, lines) {
...
function generateTestRecommendations(missingTests, coverageRatio) {
    ...
    if (missingTests.length > 0) {
        const highPriorityFiles = missingTests.filter(f => f.includes('/services/') || f.includes('/api/') || f.includes('/utils/'));
